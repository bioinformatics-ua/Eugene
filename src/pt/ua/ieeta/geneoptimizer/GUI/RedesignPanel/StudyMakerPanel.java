...
public class StudyMakerPanel extends ContentPanel
{
    ...
    private static StudyMakerPanel instance = null;
    ...
    public synchronized static StudyMakerPanel getInstance()
    {
        if (instance == null)
        {
...
//        this.setLayout(new BorderLayout());
        ...
        if (DEBUG) this.setBorder(BorderFactory.createLineBorder(Color.red, 1));

        /* Add host choice to the top of the layout. */
        JPanel hostSelection = HostSelectionPanel.getInstance();
        if (DEBUG) hostSelection.setBorder(BorderFactory.createLineBorder(Color.green, 1));
//        this.add(hostSelection, BorderLayout.NORTH);
        ...
        if (DEBUG) redesignPlugins.setBorder(BorderFactory.createLineBorder(Color.green, 1));
        redesignPlugins.add(accordionPanel);
        if (DEBUG) accordionPanel.setBorder(BorderFactory.createLineBorder(Color.blue, 1));
...
//        this.add(redesignPlugins, BorderLayout.CENTER);
        ...
        if (DEBUG) buttonsPanel.setBorder(BorderFactory.createLineBorder(Color.green, 1));
